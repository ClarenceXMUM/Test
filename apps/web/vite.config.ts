...
const isNeutralino = process.env.VITE_TARGET === `neutralino`

const base = isNetlify || isCfWorkers || isCfPages ? `/` : isUTools || isNeutralino ? `./` : `/md/`
...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    define: {
      process,
      ...(isNeutralino ? { 'window.__MD_NEUTRALINO__': 'true' } : {}),
    },
    envPrefix: [`VITE_`, `CF_`],
    plugins: [
      vue(),
    ...
    build: {
      ...
      ...(isNeutralino ? { outDir: `../neutralino/resources/dist`, emptyOutDir: true } : {}),
    },
  }
})
